...
def prep_for_nc(agg_obj, loc_dim="poly_idx"):
    ...
    # there.
    ds_out = xr.Dataset(
        coords={"poly_idx": (["poly_idx"], agg_obj.agg.poly_idx.values)}
    )

    # Add other polygon attributes
    for var in [
        c
        for c in agg_obj.agg.columns
        if c not in ["poly_idx", "rel_area", "pix_idxs", "coords"]
    ]:
        if var not in agg_obj.ds_in.var():
            ...
            ds_out[var] = xr.DataArray(
                data=agg_obj.agg[var], coords=[agg_obj.agg.poly_idx], dims=["poly_idx"]
            )
        else:
            ...
            ds_out[var] = xr.DataArray(
                data=np.zeros(
                    (
                        len(agg_obj.agg),
                        *[
                            agg_obj.ds_in[var].sizes[k]
                            for k in agg_obj.ds_in[var].sizes.keys()
                            if k not in ["lat", "lon", "loc"]
                        ],
                    )
                )
                * np.nan,
                ...
                coords=[
                    [k for k in agg_obj.agg.poly_idx],
                    *[
                        agg_obj.ds_in[var][k].values
                        for k in agg_obj.ds_in[var].sizes.keys()
                        if k not in ["lat", "lon", "loc"]
                    ],
                ],
            ...
            for poly_idx in agg_obj.agg.poly_idx:
                ds_out[var].loc[{"poly_idx": poly_idx}] = agg_obj.agg.loc[
                    poly_idx, var
                ][0]
    ...
    for crd in [
        k
        for k in agg_obj.ds_in.sizes.keys()
        if (k not in ["lat", "lon", "loc", "bnds"])
    ]:
        ds_out[crd] = xr.DataArray(
            dims=[crd],
...
def prep_for_csv(agg_obj, return_geodataframe=False):
    ...
    for var in [
        ...
        for c in agg_obj.agg.columns
        if (
            (c not in ["poly_idx", "rel_area", "pix_idxs", "coords"])
            & (c in agg_obj.ds_in.var())
        )
    ]:
        ...
        dimsteps = [
            agg_obj.ds_in[var][d].values
            for d in agg_obj.ds_in[var].sizes.keys()
            if d not in ["lat", "lon", "loc"]
        ]
        ...
        if len(dimsteps) == 0:
            ...
            # one value per row)
            expanded_var = pd.DataFrame(
                pd.DataFrame(csv_out[var].to_list())[0].to_list(), columns=[var]
            )
        else:
            expanded_var = pd.DataFrame(
                pd.DataFrame(csv_out[var].to_list())[0].to_list(),
                columns=[
                    var + str(idx) for idx in np.arange(0, len(csv_out[var][0][0]))
                ],
            )
        # Append to existing series
        csv_out = pd.concat([csv_out.drop(columns=(var)), expanded_var], axis=1)
        del expanded_var
